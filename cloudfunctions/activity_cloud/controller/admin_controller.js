...
class AdminController extends CCMiniController {
	...
	async getMeetList() {
		...
		for (let k in list) {
			list[k].MEET_STATUS_DESC = MeetModel.getDesc('STATUS', list[k].MEET_STATUS);
			...
			list[k].MEET_EXPIRE_TIME = ccminiTimeUtil.timestamp2Time(list[k].MEET_EXPIRE_TIME, 'Y-M-D');
		}
		result.list = list;
	...
	async getMeetDetail() {
		...
		let service = new AdminService();
		let meet = await service.getMeetDetail(input.id);
	...
	async delMeet() {
		...
		await service.delMeet(input.uid, input.id);
	...
	async delJoin() {
		...
		let service = new MeetService();
		await service.cancelJoin(input.uid, input.id);
	...
	async sortMeet() { // 数据校验
		...
		let service = new AdminService();
		await service.sortMeet(input.id, input.sort);
	}
	...
	async statusMeet() {
		...
		let service = new AdminService();
		await service.statusMeet(input.id, input.status);
	}
	/************** 活动 END ********************* */
